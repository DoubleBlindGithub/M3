...
def process_partition(args, definitions, code_to_group, id_to_group, group_to_id,
                      ...
                      eps=1e-6, future_time_interval=24.0, fixed_hours=48.0, min_los = 24.0):
    ...
    for (patient_index, patient) in enumerate(patients):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as ts_file:
                ...
                los_masks.append(los_mask)
                los_labels.append(los_label)
                los_positions.append(los_position)
                ...
                # create readmission
                next_admission_days = label_df.iloc[0]["Readmission"]
                if 0.0<float(next_admission_days)<7.0 :
                    readmit_label = 1
                elif 7.0<=float(next_admission_days)<30.0:
                    readmit_label = 2
                elif 30.0<=float(next_admission_days)<90.0:
                    readmit_label = 3
                elif 90.0<=float(next_admission_days)<365.0:
                    readmit_label = 4
                else:
                    readmit_label = 0
                
                # mask patient who died, there is no point to predict readmission if a patient died in icu stay
                readmit_mask = 1
                if mortality:
                    readmit_mask = 0

                readmit_labels.append(readmit_label)
                readmit_masks.append(readmit_mask)

                # create long term mortality
                survival = label_df.iloc[0]['Survival']
                survival = pd.Timedelta(survival)
                if (survival < pd.Timedelta('365 days')) & (survival > pd.Timedelta('0 days')):
                    ltm_label = 1
                else:
                    ltm_label = 0
                ...
                ltm_labels.append(ltm_label)
                ltm_masks.append(ltm_mask)
    ...
    los_positions = permute(los_positions, perm)
    ...
    readmit_labels = permute(readmit_labels, perm)
    readmit_masks = permute(readmit_masks, perm)

    ltm_labels = permute(ltm_labels, perm)
    ltm_masks = permute(ltm_masks, perm)

    with open(os.path.join(output_dir, "listfile.csv"), "w") as listfile:
        header = ','.join(['filename', 'length of stay', 'in-hospital mortality task (pos;mask;label)',
                           'length of stay task (pos;masks;labels)', 'phenotyping task (labels)',
                           'decompensation task (masks;labels)', 'readmission task (masks;labels)', 'long-term mortality task (mask; label)'])
        ...
        for index in range(len(file_names)):
            ...
            los_task = '{};{};{}'.format(los_positions[index], los_masks[index], los_labels[index])
            ...
            readmit_task = '{};{}'.format(readmit_masks[index], readmit_labels[index])
            ltm_task = '{};{}'.format(ltm_masks[index], ltm_labels[index])

            listfile.write(','.join([file_name, los, ihm_task, los_task, pheno_task, decomp_task, readmit_task, ltm_task]) + "\n")
