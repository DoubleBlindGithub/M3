...
def add_next_admission(admits):
    """
    add next admission time and also next admission type
    """
    admits = admits.sort_values(by=['SUBJECT_ID','ADMITTIME'])
    ...
    elective_rows = admits.NEXT_ADMISSION_TYPE == 'ELECTIVE'
    admits.loc[elective_rows,'NEXT_ADMITTIME'] = pd.NaT
    admits.loc[elective_rows,'NEXT_ADMISSION_TYPE'] = np.NaN
...
def filter_inconsistent_deathtime(stays):
    stays['DOD_DATE'] = stays['DOD'].dt.date
    stays['DEATHTIME_DATE'] = stays['DEATHTIME'].dt.date
    stays['DEATH_DIFF'] = stays['DOD_DATE']- stays['DEATHTIME_DATE']
    stays = stays[(abs(stays.DEATH_DIFF) == pd.Timedelta('0 days')) | stays.DEATH_DIFF.isnull()]
    return stays

def add_outhospital_mortality_to_icustays(stays):
    stays['DISCH_DATE'] = stays['DISCHTIME'].dt.date
    stays['SURVIVAL'] = stays['DOD_DATE'] - stays['DISCH_DATE']
    return stays 
