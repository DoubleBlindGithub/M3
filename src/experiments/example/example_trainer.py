...
if not(os.path.exists('runs')):
    print("Making directory runs to store logs")
    os.makedirs('runs')
if not(os.path.exists('ckpt')):
    print("Making directory ckpt to store weights")
    os.makedirs('ckpt')
...
encoder = MultiModalEncoder(ts_model = ts_model, 
...
decomp_model = FCTaskComponent("decomp", None, global_dim, 2, 
                                hidden_size = 128)
ihm_model = FCTaskComponent("ihm", None, global_dim, 2, 
                                hidden_size = 128)
los_model = FCTaskComponent("los", None, global_dim, 3, 
                                hidden_size = 32)
pheno_model = FCTaskComponent("pheno", None, global_dim, 25, 
                                hidden_size = 128)
readmit_model = FCTaskComponent("readmit", None, global_dim, 5, 
                                hidden_size = 128)
ltm_model = FCTaskComponent("ltm", None, global_dim, 2, 
                                hidden_size = 128)
...
model = MultiModalMultiTaskWrapper(mm_encoder = encoder, \
    ...
    pheno_model = pheno_model, readmit_model = readmit_model, 
    ltm_model = ltm_model)


#------------------------- Logs and optimizer if using uncertainty weighting --#
log_var = {
