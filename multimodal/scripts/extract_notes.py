...
df2 = df2[(df2.CHARTTIME.notnull()) | (df2.CATEGORY == 'Discharge summary')]
df2 = df2[df2.TEXT.notnull()]

# used for readmission tasks
    ...
    for folder in all_folders:
        ...
        try:
            ...
            for ind, hid in enumerate(hadm_ids):
                ...
                cur_stay = sliced[sliced.HADM_ID == hid]
                if len(cur_stay) == 0:
                    continue
                # text = sliced.TEXT.str.cat(sep=' ')
                # text = "*****".join(list(preprocess_mimic(text)))
                data_json = {}
                for index, row in cur_stay.iterrows():
                    if pd.isnull(row['CHARTTIME']):
                        data_json["ds_{}".format(hid)
